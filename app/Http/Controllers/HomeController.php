...
class HomeController extends Controller
{
    ...
    */
    ...
    public function dashboard()
    {
        ...
        $facturas = $oSincronizacionControlador->buscarFacturaReturn($fechainicioMes, $fechaActual, 0);
        $facturas =$facturas->values;
        foreach ($anEstadosSiat as $item) {
            //$facturas = $oSincronizacionControlador->buscarFacturaReturn($fechainicioMes, $fechaActual, $item);
            //$facturasFiltradas =$facturas->values;
            $facturasFiltradas;
            if(filled($facturas)){
                $facturasFiltradas = array_filter($facturas, function ($factura) use($item){
                    return $factura->EstadoEnSiat === $item;
                });
            }else{
